        ...
        <div class="code-formatted">
          ...
          <div v-for="(bullet, bulletIndex) in image.bullets" :key="bullet.id">
            <span class="class-name">.lam-bullet-{{ imageIndex + 1 }}-{{ bulletIndex + 1 }}</span> {
              <div>&nbsp;&nbsp;<span class="property-name">top</span>: {{ bullet.top }}%;</div>
              <div>&nbsp;&nbsp;<span class="property-name">left</span>: {{ bullet.left }}%;</div>
            }
          </div>
        </div>
      </div>
    ...
    methods: {
      copyRaw(bullets, imageIndex) {
        ...
        bullets.forEach((el, bulletIndex) => {
          res += `.lam-bullet-${imageIndex + 1}-${bulletIndex + 1}{top:${el.top}%;left:${el.left}%;}`
        });
